    ...
    <div className="sticky top-0 z-50 flex justify-between items-center bg-white px-6 py-3 shadow">
      ...
      <div className="flex items-center gap-6 relative">
        ...
        <div className="relative">
          ...
          {/* Notification Popup */}
          {showNotifications && (
            <div className="absolute right-0 mt-2 w-[360px] bg-white shadow-xl rounded-xl p-4 z-50">
              <h2 className="text-lg font-bold mb-4">Notification</h2>
              {notifications.map((item) => (
                <div className="relative border-l-2 border-dotted border-gray-300 pl-6 mb-6" key={item.id}>
                  <span className={`absolute left-[-7px] top-1 w-4 h-4 rounded-full ${item.color}`}></span>
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-xs text-gray-500 bg-gray-100 rounded px-2 py-0.5">{item.date}</span>
